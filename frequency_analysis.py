...
def frequency_analysis(input_text: str, output_text: str):
    """Perform frequency analysis on the given input and output texts and display a bar graph of the frequency difference."""
    ...
    frequency_difference = [output_frequencies[i] - input_frequencies[i] for i in range(len(characters))]
    
    plt.figure(figsize=(10, 6))
    plt.bar(characters, frequency_difference, color='blue')
    plt.xlabel('Characters')
    plt.ylabel('Frequency Difference')
    plt.title('Frequency Analysis (Output - Input)')
    plt.show()